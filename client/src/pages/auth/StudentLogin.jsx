...
function StudentLogin() {
  ...
  const onSubmit = async () => {
    ...
    setLoading(true);
    try {
      const res = await loginUser({ email, password, role: "student" });
      const data = res?.data ?? {};
      const token = data?.token ?? data?.accessToken ?? data?.jwt;

      if (token) localStorage.setItem("token", token);
      localStorage.setItem("role", "student");
      auth?.login?.(data?.user ?? { email, role: "student" });
      ...
      <div className="card w-full max-w-md">
        <div className="card-pad">
          <div className="text-center">
            <h2 className="text-2xl sm:text-3xl font-semibold tracking-tight">Student login</h2>
            <p className="muted mt-2">Sign in to your student account.</p>
          </div>
